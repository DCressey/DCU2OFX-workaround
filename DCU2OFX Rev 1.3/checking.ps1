<#
.NOTES
    Script: checking.ps1         Rev:  1.3
    Author: dcressey             Date: 5/8/22
    
.SYNOPSIS
    Handles checking transactions for DCU2OFX
...
# Note that Checks are negative numbers
...
$balance    = '1000.0'        #The balance before these transactions (neg for loans)
...
Import-csv  $input  | % {
    ...
    $FITID = $_.ID.GetHashcode().ToString('X')
    $NAME = $_.DESCRIPTION.PadRight(20).Substring(0, 20)
